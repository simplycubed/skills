...
| **`provision.yml`** | manual (`workflow_dispatch`) | `prod` | One-time infra: create the R2 bucket (`r2-bootstrap.mjs`), optionally copy objects from an old bucket during a rename, deploy the CDN Worker, bring up `cdn.simplycubed.com`, smoke-test. `whoami` preflight + cert-propagation retry. |
...
1. `provision.yml` on `main` with `copy_from_bucket=simplycubed-skills`, `copy_dry_run=true`.
2. `provision.yml` again on `main` with `copy_from_bucket=simplycubed-skills`, `copy_dry_run=false`.
3. `provision.yml` from the rename branch/PR head with no copy input, so only the Worker binding flips.
