...
// content-addressed off the manifest's contentHash, so the 110 active manifests
// name all 220 objects without ever listing the bucket.
//
...
for (const cfg of skills) {
  ...
  try {
    for (const { key, type, verify } of units) {
      ...
      // Fail-closed: the blob is content-addressed, so the bytes we just read must
      // re-hash to exactly what the committed manifest attests. Anything else is a
      // corrupt source object and must not reach the new bucket.
      if (verify) {
